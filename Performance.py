    ...
    with open(predfile, "r") as f:
        ...
        for line in f:
            ...
            if len(v) >= 3:  # Ensure the line has at least three elements
                ...
                v[2] = int(v[2]) #label -> we use int instead of bool because is useful after for the confusion matrix (we use them as indexes to access the matrix)
                preds.append(v)
            else:
                print("yes")
                print(line, c)
...
if __name__=="__main__":
    ...
    if "com" in sys.argv[1]:
        print(">" + " " + str(th))
        print('TP=' + str(cm[1][1]) + "," + 'TN=' + str(cm[0][0]) + "," + 'FN=' + str(cm[1][0]) + "," + 'FP=' + str(cm[0][1]) + "," + 'Q2=' + str(q2) + "," + 'MCC=' + str(mcc) + "," + 'F1=' + str(f1))
    else:
