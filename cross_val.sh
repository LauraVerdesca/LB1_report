...
# # NEGATIVE
# # Step 1: Randomize and split the IDs
# zcat -f swiss_negatives.fasta.gz | grep "^>" | cut -d  "|" -f 2 > negative.ids
# sort -R negative.ids > negatives_r1.ids
# split -d -a 1 -n 5 negatives_r1.ids neg_subset_

# # Step 2: Create the 5 negative fasta files
# for i in {0..4}; do
#     python3 select_fasta.py  neg_subset_$i  <(zcat -f swiss_negatives.fasta.gz) 2   > neg_subset_$i.fasta
# done
